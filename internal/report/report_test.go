...
func TestFindClusterPolicyReportResult(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		report.AddResult(&policy, namespaceResource, &admReviewPass, nil)
		expectedResource = namespaceResource
	}

	result, ok := report.GetResult(&policy, expectedResource)
	if !ok {
		t.Errorf("Policy result not found")
	}
	if result == nil {
...
func TestCopyPolicyReportResults(t *testing.T) {
	time := metav1.Now()
	...
	for _, ttest := range tests {
		t.Run(ttest.name, func(t *testing.T) {
			...
			clusterReport.CopyResult(ttest.result)
			nsReport.CopyResult(ttest.result)
